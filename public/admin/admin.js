...
async function setupAdminPushNotifications() {
    ...
    try {
        ...
        if (sub) {
            ...
            if (btn) btn.innerHTML = '✅ Alerts Enabled';
        }
...
async function subscribeToAdminPush() {
    ...
    try {
        ...
        if (saveRes.ok) {
            showToast('Admin alerts enabled! 🔔');
            const btn = document.getElementById('admin-push-btn');
            if (btn) btn.innerHTML = '✅ Alerts Enabled';
        } else {
        ...
        showToast('Notification permission denied or error occurred.');
    }
}

document.addEventListener('DOMContentLoaded', setupAdminPushNotifications);
